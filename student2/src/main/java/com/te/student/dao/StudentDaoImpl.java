...
//		EntityManagerFactory factory = Persistence.createEntityManagerFactory("springdb");
	...
	public StudentBean getDetailsBasedOnName(String name) {
		...
		EntityManager entityManager = factory.createEntityManager();
		try {
		String query = "from StudentBean where name=:name";
		Query query2 = entityManager.createQuery(query);
		query2.setParameter("name", name);
		StudentBean getGradeData = (StudentBean) query2.getSingleResult();          //getResultList
           return getGradeData;
		}
		catch (Exception e) {
			return null;
